...
const ContactForm = () => {
	...
	const validateForm = () => {
		...
		if (!formData.phone.trim()) {
			...
			if (formData.phone.startsWith("+998")) {
				...
				newErrors.phone =
					"Неверный формат номера. Либо +998 90 123 45 67, либо локально: 90 123 45 67";
			}
		}
	...
	const handleSubmit = async (e) => {
		...
		toast({
			...
			description: "Пожалуйста, подождите пока мы обрабатываем вашу заявку.",
		});
					...
					>
						...
						<form onSubmit={handleSubmit} className="space-y-6">
							...
							{showSuccessBanner ? (
								<motion.div
									key="submit-success"
									className="w-full bg-green-600 text-white py-3 px-6 rounded-lg font-semibold flex items-center justify-center gap-2 transition-all"
					...
					>
						<div>
							...
							<p className="text-gray-300 mb-8 leading-relaxed">
								Предпочитаете прямое общение? Мы всегда готовы ответить на ваши
								вопросы и обсудить ваш проект удобным для вас способом.
							</p>
						</div>
							...
							>
								...
								<div>
									...
									<p className="text-gray-300">
										{mockData.contact.social.telegram}
									</p>
